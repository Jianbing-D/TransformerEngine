...
from transformer_engine.common.cutedsl.linear_cross_entropy import scheduler as tile_scheduler
...
class FwdMainLoop:
    ...
    def kernel(
        ...
        rank: cutlass.Int32,
        scheduler_params: tile_scheduler.ParamsBase,
    ):
        """
        ...
        # -------- SMEM partition (shared, outside all warp-group blocks) ------------ #
        ...
        # Static k-loop count (dim is a compile-time constant per JIT specialization)
        num_k_tiles: cutlass.Int32 = cute.ceil_div(problem_mnk[2], self.mma_tiler[2])

        # ---------- CTA layout for TMA (static, shared) ----------- #
        ...
        # Allocate TMEM once (all tiles reuse the same TMEM)
        ...
        # [(tileM, tileN), loopM, loopN] — TMEM tensor layout is fixed
        ...
        # Tile scheduler factory (shared)
        TileSchedulerCls = partial(
            tile_scheduler.StaticPersistentScheduler.create, scheduler_params
        )

        # ========================================================
        # Per-warp-group persistent loops.
        # Each warp group creates its own scheduler instance and
        # processes all assigned tiles in a while loop. Pipeline
        # barriers keep load / MMA / epilogue in lock-step.
        # ========================================================

        # ///////
        # empty warps — no per-tile work
        # ///////
        ...
        if warp_idx == self.load_warp_ids:
            ...
            load_scheduler = TileSchedulerCls()
            load_work = load_scheduler.initial_work_tile_info()
            while load_work.is_valid_tile:
                pidm_load, pidn_load = load_work.tile_idx

                block_vocab_right_idx_load: cutlass.Int64 = min(
                    (pidn_load + 1) * self.vocab_per_split, problem_mnk[1]
                )
                num_n_tiles_load: cutlass.Int64 = cute.ceil_div(
                    block_vocab_right_idx_load - pidn_load * self.vocab_per_split,
                    self.mma_tiler[1],
                )

                # [tileM, tileK, loopK]
                gA_load = cute.local_tile(
                    mA, (self.mma_tiler[0], self.mma_tiler[2]), (pidm_load, None)
                )
                # [vocab_size_per_split, dim]
                mB_n_load = cute.local_tile(
                    mB,
                    (self.vocab_per_split, cute.size(mB.layout.shape, mode=[1])),
                    (pidn_load, 0),
                )
                # [tileN, tileK, loopN, loopK]
                gB_load = cute.local_tile(
                    mB_n_load, (self.mma_tiler[1], self.mma_tiler[2]), (None, None)
                )
                tCgA_load = thr_mma.partition_A(gA_load)
                tCgB_load = thr_mma.partition_B(gB_load)
                ...
                tTMAsA, tTMAgA = cpasync.tma_partition(
                    ...
                    cute.group_modes(tCgA_load, 0, 3),
                )
                tTMAsB, tTMAgB = cpasync.tma_partition(
                    ...
                    cute.group_modes(tCgB_load, 0, 3),
                )

                for n in cutlass.range(num_n_tiles_load):
                ...
                load_scheduler.advance_to_next_work()
                load_work = load_scheduler.get_current_work()
        ...
        if warp_idx == self.mma_warp_ids:
            ...
            mma_scheduler = TileSchedulerCls()
            mma_work = mma_scheduler.initial_work_tile_info()
            while mma_work.is_valid_tile:
                pidn_mma = mma_work.tile_idx[1]

                block_vocab_right_idx_mma: cutlass.Int64 = min(
                    (pidn_mma + 1) * self.vocab_per_split, problem_mnk[1]
                )
                num_n_tiles_mma: cutlass.Int64 = cute.ceil_div(
                    block_vocab_right_idx_mma - pidn_mma * self.vocab_per_split,
                    self.mma_tiler[1],
                )

                for n in cutlass.range(num_n_tiles_mma):
                ...
                mma_scheduler.advance_to_next_work()
                mma_work = mma_scheduler.get_current_work()
        ...
        if warp_idx in self.epi_warp_ids:
            ...
            # ---- Epilogue fixed setup (hoisted outside per-tile loop) ---- #
            ...
            # ---- Per-tile persistent loop ---- #
            epi_scheduler = TileSchedulerCls()
            epi_work = epi_scheduler.initial_work_tile_info()
            while epi_work.is_valid_tile:
                pidm, pidn = epi_work.tile_idx
                ...
                num_n_tiles_epi: cutlass.Int64 = cute.ceil_div(
                    block_vocab_right_idx - block_vocab_left_idx, self.mma_tiler[1]
                )

                # auxiliary tensors (per tile)
                # [tileM]
                ...
                tLabelsCAcc_mask[0] = cute.elem_less(
                    pidm * self.epi_tile[0] + tidx, problem_mnk[0]
                )
                ...
                tR2GrLogprobs = cute.make_fragment(
                    tR2GgLogprobs.shape, tR2GgLogprobs.element_type
                )
                ...
                for n in cutlass.range(num_n_tiles_epi):
                ...
                epi_scheduler.advance_to_next_work()
                epi_work = epi_scheduler.get_current_work()

        # Dealloc TMEM once, after all warp-group while loops complete
        self.cta_sync_barrier.arrive_and_wait()
        if warp_idx == self.empty_warp_ids[0]:
    ...
    def _compute_grid(
        ...
        num_splits: int,
        occupancy: int = 1,
    ) -> Tuple[Tuple[int, int, int], "tile_scheduler.StaticPersistentScheduler.Params"]:

        cluster_shape = (*cluster_shape_mn, 1)

        num_m_tiles = cute.ceil_div(problem_mnk[0], cta_tiler[0])
        sched_params_init = tile_scheduler.TileSchedulerParams(
            num_tiles_M=cutlass.Int32(num_m_tiles),
            num_tiles_N=cutlass.Int32(num_splits),
        )
        sched_params = tile_scheduler.StaticPersistentScheduler.to_underlying_arguments(
            sched_params_init
        )
        grid = tile_scheduler.StaticPersistentScheduler.get_grid_shape(
            sched_params, occupancy=occupancy
        )
        grid = cute.round_up(grid, cluster_shape)
        return grid, sched_params
    ...
    def __call__(
        ...
        grid, sched_params = self._compute_grid(
