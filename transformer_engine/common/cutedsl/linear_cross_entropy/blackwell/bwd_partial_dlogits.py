...
class BwdPartialDlogits:
    ...
    def kernel(
        ...
        """
        The backward kernel for partial d_logits (vanilla grid scheduling).
        """
        ...
        cluster_m_size = cute.size(tiled_mma.thr_id.shape)
        mma_tile_coord_v = bidx % cluster_m_size
        is_leader_cta = mma_tile_coord_v == 0

        # Tile indices from grid position (vanilla scheduling)
        pidm = bidx // cluster_m_size
        pidn = bidy
        pidm_cta = pidm * cluster_m_size + mma_tile_coord_v
    ...
    def __call__(
        ...
        grid = self._compute_grid(problem_mnk, self.cluster_shape_mn, self.cta_tile_shape_mnk)
