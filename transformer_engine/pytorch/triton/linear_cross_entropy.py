...
@triton.jit
def efficient_entropy_forward_num_valid_tokens(
    num_tokens,
    ignore_index,
    labels_ptr,
    ...
    for m in range(0, num_pid_m):
        ...
        num_valid_tokens += tl.sum(valid_labels_mask.to(tl.int32), axis=0)
...
def efficient_entropy_kernel_general_mainloop(
    ...
    for n in range(0, num_pid_n):
        ...
        # Set label_mask to 0 if labels equal ignore_index
        label_mask &= valid_labels_mask[:, None]
        _logprobs += tl.sum(logits * tl.where(label_mask, 1.0, 0.0), axis=1)
...
def efficient_entropy_triton_kernel_epilogue(
    ...
    if reduction == 0:  # no-reduction
        ...
        global_logprobs_scalar = tl.sum(global_logprobs, axis=0) / num_valid_tokens.to(tl.float32)
        tl.atomic_add(global_logprobs_scalar_ptr, global_logprobs_scalar)
...
@triton.jit
def efficient_entropy_triton_epilogue_tp_logprobs(
    num_tokens,
    ignore_index,
    num_valid_tokens_ptr,
    labels_ptr,
    ...
    if reduction == 0:  # none
        ...
        num_valid_tokens = tl.load(num_valid_tokens_ptr)
        logprobs_scalar = tl.sum(logprobs, axis=0) / num_valid_tokens.to(tl.float32)
        tl.atomic_add(logprobs_scalar_ptr, logprobs_scalar)
...
def efficient_entropy_forward(
    ...
    num_valid_tokens = torch.zeros((), device=hidden.device, dtype=torch.int32)
    assert maximum.is_contiguous() and accumulate.is_contiguous()

    # intermediate buffers
    vocab_per_split = 1024
    assert vocab_per_split % 256 == 0 and vocab_per_split >= 256
    num_splits = (vocab_size + vocab_per_split - 1) // vocab_per_split
    ...
    efficient_entropy_forward_num_valid_tokens[(1,)](
...
@triton.autotune(
    configs=[
        triton.Config(
            {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 64, "GROUP_SIZE_M": 16},
            num_stages=3,
            num_warps=4,
        )
    ],
...
@triton.jit
def efficient_entropy_backward_d_logits(
    num_tokens,
    hidden_size,
    vocab_size,
    ignore_index,
    ...
    maximum = tl.load(maximum_ptr + offs_am * stride_maximum, mask=offs_am < num_tokens)
    accu = tl.load(
    ...
    if reduction == 0:  # none
        d_logprobs = tl.load(
            d_logprobs_ptr + offs_am * stride_d_logprobs, mask=offs_am < num_tokens
        )
    elif reduction == 1:  # sum
        d_logprobs = tl.broadcast_to(tl.load(d_logprobs_ptr), (BLOCK_SIZE_M,))
    else:  # mean
        num_valid_tokens = tl.load(num_valid_tokens_ptr)
        d_logprobs = tl.broadcast_to(
            tl.fdiv(tl.load(d_logprobs_ptr), num_valid_tokens.to(tl.float32)), (BLOCK_SIZE_M,)
        )
    ...
    valid_labels_mask = labels != ignore_index
    # d_logprobs *= valid_labels_mask
    d_logprobs = tl.where(valid_labels_mask, d_logprobs, 0.0)
...
@triton.jit
def efficient_entropy_backward_kernel_d_logits_split_N(
    split_idx,
    num_tokens,
    hidden_size,
    vocab_size,
...
def efficient_entropy_backward(
    ...
    assert _config._backward in (BackwardEnum._Split_Dlogits_N, BackwardEnum._Total_Separate)
    ...
    if _config._backward == BackwardEnum._Total_Separate:
        ...
        for split_idx in range(num_splits):
            efficient_entropy_backward_kernel_d_logits_split_N[d_logits_grid](
                split_idx,
                num_tokens,
                hidden_size,
                vocab_size,
