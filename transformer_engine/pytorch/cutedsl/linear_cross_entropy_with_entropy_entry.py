...
from transformer_engine.common.triton import linear_cross_entropy_with_entropy as triton_entropy_kernels
...
def forward(
    ...
    if _get_fwd_config()._fwd_mainloop_kernels.get(key) is None:
        fwd_mainloop_kernel = _get_impl().gpu_entry.fwd_mainloop_entropy.FwdMainLoopEntropy(
...
def backward(
    ...
    if _backward_method == utils.BackwardMethodEnum.kDlogitsSplitN:
        ...
        if _get_bwd_config()._bwd_kernel.get(key) is None:
            bwd_kernel = _get_impl().gpu_entry.bwd_partial_dlogits_entropy.BwdPartialDlogitsEntropy(
