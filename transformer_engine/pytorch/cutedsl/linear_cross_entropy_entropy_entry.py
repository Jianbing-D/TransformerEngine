...
from transformer_engine.common.triton import linear_cross_entropy_entropy as triton_entropy_kernels
...
def forward(
    ...
    assert _entropy_b.is_contiguous() and entropy_b.is_contiguous() and entropy.is_contiguous()
    ...
    _entropy_b_packed = from_dlpack(_entropy_b, assumed_align=8).mark_compact_shape_dynamic(
        mode=0, stride_order=(0, 1)
    )
    ...
    key = f"entropy+vocab_size:{vocab_size}+dim:{dim}+dtype:{hidden_view.dtype}"
    if _get_fwd_config()._fwd_mainloop_kernels.get(key) is None:
        fwd_mainloop_kernel = _get_platform().gpu_entry.fwd_mainloop_entropy.FwdMainLoopEntropy(
    ...
    if not in_tp_mode:
        ...
        triton_entropy_kernels.forward_dp_epilogue_entropy[grid](
            num_tokens,
            num_splits,
            ignore_index,
            labels_view,
        ...
        triton_entropy_kernels.forward_tp_epilogue_entropy[grid](
            num_tokens,
            num_splits,
            _max,
            _max.stride(0),
        ...
        # reduce accumulate and entropy_b together
        accu_and_eb = torch.stack([accumulate, entropy_b])
        dist.all_reduce(accu_and_eb, op=dist.ReduceOp.SUM, group=tp_group)
        accumulate = accu_and_eb[0]
        entropy_b = accu_and_eb[1]

        # update logprobs, convert accumulate to LSE, compute entropy
        torch.cuda.current_stream().wait_event(_get_fwd_config()._dedicated_events[1])
        triton_entropy_kernels.forward_tp_epilogue_update_logprobs_entropy[grid](
            num_tokens,
            ignore_index,
            num_valid_tokens,
            labels_view,
...
def backward(
    ...
    """
    backward host function with entropy gradient
    """
    in_tp_mode = (tp_group is not None) and (tp_world_size > 1)
    ...
    if _backward_method == utils.BackwardMethodEnum.kDlogitsSplitN:
        ...
        dentropy_packed = from_dlpack(
            dentropy_view.detach(), assumed_align=8
        ).mark_compact_shape_dynamic(mode=0)
        accu_packed = from_dlpack(accu.detach(), assumed_align=8).mark_compact_shape_dynamic(
            mode=0
        )
        entropy_b_packed = from_dlpack(entropy_b.detach(), assumed_align=8).mark_compact_shape_dynamic(
            mode=0
        )
        ...
        key = (
            f"entropy+vocab_size:{vocab_size}+dim:{dim}+reduction:{REDUCTION}+dtype:{hidden_view.dtype}"
        )
        if _get_bwd_config()._bwd_kernel.get(key) is None:
            bwd_kernel = _get_platform().gpu_entry.bwd_partial_dlogits_entropy.BwdPartialDlogitsEntropy(
    ...
    # convert d_hidden to the same dtype as global_hidden
